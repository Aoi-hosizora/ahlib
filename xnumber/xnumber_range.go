...
// checkSignedRange checks whether given signed integer from, to and step arguments is legal to represent a range, and also calculates the range slice capacity.
func checkSignedRange(from, to, step int64) (capacity int, legal bool) {
	if step > 0 && from < to {
...
// checkUnsignedRange checks whether given unsigned integer from, to and step arguments is legal to represent a range, and also calculates the range slice capacity.
func checkUnsignedRange(from, to, step uint64, reverse bool) (capacity int, legal bool) {
	if step != 0 && !reverse /* step > 0 */ && from < to {
		return int(math.Ceil(float64(to-from) / float64(step))), true
	}
	if step != 0 && reverse /* step < 0 */ && from > to {
		return int(math.Ceil(float64(from-to) / float64(-step))), true
