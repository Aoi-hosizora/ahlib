...
func TestSet(t *testing.T) {
	...
	assert.Equal(t, xslice.ItsOfInt(set.Slice()), []int{1, 2, 3, 4})

	set.Remove(1, 4)
	assert.Equal(t, xslice.ItsOfInt(set.Slice()), []int{2, 3})

	set.Clear()
	assert.Equal(t, set.Size(), 0)

	set = FromSlice(xslice.Sti([]int{1, 5, 2, 1, 3, 5, 4}))
	assert.Equal(t, xslice.ItsOfInt(set.Slice()), []int{1, 5, 2, 3, 4})
